...
void usartInit(USART_TypeDef *usart,uint32_t br, int setremap){
    ...
    GPIOB->CRL   |= (0x04UL  << 28); }           // RX  (PB7)
	  ...
	  if(setremap==0)                    {
		RCC->APB2ENR |= 0x01;   //alternate enable (AFIO)
	  RCC->APB2ENR |= 1<<2;												  	// enable clock for GPIOA		
...
/** @brief     Recieve String.
		...
		@param[out] urt->DR i.e USASRT1->DR; \f
								return value of the incoming character
		@example  GetString(USART1,&buffer);
*/
void GetString (USART_TypeDef *uart,char *data)  {
	int i=0; 	char temp=0;
...
//*******************************
 unsigned char RecvString(USART_TypeDef *uart,unsigned char *x, unsigned char size)
{
    unsigned char i = 0;
